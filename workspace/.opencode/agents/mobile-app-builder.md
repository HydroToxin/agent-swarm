...
permission:
  ...
  bash: "allow"  # May run build/test commands for mobile projects
  task: "allow"
---
...
- If requirements are unclear (platform choice, min OS versions, offline scope), stop and request clarification via the Orchestrator/PM.
- Do not add features “because mobile apps usually need it” unless the spec explicitly requires it.

### Application root (`APP_ROOT`)

Use **`APP_ROOT`** from the orchestrator prompt (usually **`$WORKSPACE/$PROJECT`**, or the mobile folder the spec names). **All** project files **under `APP_ROOT`**. `cd "$APP_ROOT"` before writes; **do not** scatter app sources at **`WORKSPACE`** root.

**Persistent memory (required):** After each task, **append** to **`$WORKSPACE/$PROJECT/.agent-swarm/docs/agent-memory.md`** (see **`AGENTS.md`**): timestamp, **Mobile App Builder**, 2–6 bullets.

### Evidence Package (Required for Review)

At the end of every implementation task, include evidence so the Code Reviewer can verify.
...
Examples (choose what fits the stack):

- iOS: `xcodebuild test ...` output
- Android: `./gradlew test` / `./gradlew assemble` output
- React Native/Flutter: build/test/lint outputs
- UI evidence (screenshots) only when the task is UI/UX
