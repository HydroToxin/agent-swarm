...
## Principle for JSON Data Processing
- **Rule**: The Coding Agent must generate JSON data in segments of a maximum of **1000 objects**.
- **Storage Strategy**: Each segment should be **immediately** and **appended** to the designated target file.
- **Important Note**: This rule is **mandatory** and must not be circumvented. The agent must ensure that it uses the correct target file for each data processing operation.
